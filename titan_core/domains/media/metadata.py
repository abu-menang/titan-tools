"""
titan_core.domains.media.metadata
...
from titan_core.core.ops import run_command
from titan_core.core.utils import ensure_dir
from titan_core.core.report import write_json, write_csv, timestamped_filename
...
def export_metadata(
    ...
    out_path = write_json(results, timestamped_filename("metadata", "json"))
    write_csv(results, timestamped_filename("metadata", "csv"))
    log.info(f"Metadata exported to: {out_path}")
    return out_path
...
if __name__ == "__main__":
    # Example:
    # python -m titan_core.domains.media.metadata --root ./movies --use-mkvmerge
    vid_metadata()
