...
NON_MKV_TRACK_COLUMNS: List[ColumnSpec] = [
...
FAILURE_COLUMNS: List[ColumnSpec] = [
    ColumnSpec("filename", "filename", width=40),
    ColumnSpec("failure_reason", "failure_reason", width=60),
    ColumnSpec("path", "path", width=120),
]

SKIPPED_COLUMNS: List[ColumnSpec] = [
    ColumnSpec("filename", "filename", width=40),
    ColumnSpec("skipped_reason", "skipped_reason", width=60),
    ColumnSpec("path", "path", width=120),
]
...
def vid_mkv_scan(
    ...
    for candidate in Progress(
        ...
        if not is_video and not is_subtitle:
            skipped_files.append({
                ...
                "skipped_reason": f"unsupported extension ({suffix or 'none'})",
            })
            continue

        scanned_files += 1
        mkv_files.append(candidate)
        ...
        try:
            ...
            failed_files.append({"path": str(candidate), "filename": candidate.name, "failure_reason": reason})
    ...
    # Remove matched videos/subtitles from the base lists
    matched_mkv_paths: set[Path] = set()
    for row in mkv_ext_subtitle_rows:
        opath = row.get("output_path")
        if opath:
            matched_mkv_paths.add(Path(opath).resolve())
    matched_non_mkv_paths: set[Path] = set()
    for row in external_subtitle_rows:
        opath = row.get("output_path")
        if opath:
            matched_non_mkv_paths.add(Path(opath).resolve())

    mkv_files = [p for p in mkv_files if p.resolve() not in matched_mkv_paths]
    non_mkv_video_files = [p for p in non_mkv_video_files if p.resolve() not in matched_non_mkv_paths]
    subtitle_files = [p for p in subtitle_files if str(p) in unmatched_subtitle_files]
    if normalized_batch <= 0:
    ...
    for chunk in file_chunks:
        ...
        chunk_failures = [
            {"path": str(entry.path), "filename": entry.path.name, "failure_reason": entry.failure_reason}
            for entry in chunk
    ...
    # Remove non-HEVC items from working lists
    mkv_non_hevc_paths = {Path(row["path"]).resolve() for row in non_hevc_rows}
    mkv_ext_subtitle_rows = [
        row for row in mkv_ext_subtitle_rows
        if Path(row.get("output_path", "") or "").resolve() not in mkv_non_hevc_paths
    ]
    mkv_files = [p for p in mkv_files if p.resolve() not in mkv_non_hevc_paths]

    non_mkv_non_hevc_paths = {Path(row["path"]).resolve() for row in non_mkv_non_hevc_rows if row.get("path")}
    external_subtitle_rows = [
        row for row in external_subtitle_rows
        if Path(row.get("output_path", "") or "").resolve() not in non_mkv_non_hevc_paths
    ]
    non_mkv_video_files = [p for p in non_mkv_video_files if p.resolve() not in non_mkv_non_hevc_paths]
